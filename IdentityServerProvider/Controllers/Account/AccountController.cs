...
using IdentityServerProvider;
using IdentityServer4.Models;
using IdentityServerProvider.Controllers.Account;
...
namespace IdentityServerProvider.Controllers.Account
{
    ...
    public class AccountController : Controller
    {
        ...
        private readonly IIDPUserRepository _IIDPUserRepository;
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var IsTwoFactorAuthenticationRequired = false;
                if (IsTwoFactorAuthenticationRequired)
                {
                    var redirectResult = ProccessTwoFactorAuthenticationAsync(model);
                ...
                {
                    if (_IIDPUserRepository.AreUserCredentialsValid(model.Username, model.Password))
                    {
                        ///**/var user = _users.FindByUsername(model.Username);
                        var user = _IIDPUserRepository.GetUserByUsername(model.Username);
        ...
        private  string ProccessTwoFactorAuthenticationAsync(LoginInputModel model)
        {
            if (_IIDPUserRepository.AreUserCredentialsValid(model.Username, model.Password))
            {
                var user = _IIDPUserRepository.GetUserByUsername(model.Username);
                ...
                var signin = HttpContext.SignInAsync("idsrv.2FA", new ClaimsPrincipal(Id));

                /// send code
                /// Impelement sending the code in production with SMS or Email
                /// 
                ...
                if (_interaction.IsValidReturnUrl(model.ReturnUrl) || Url.IsLocalUrl(model.ReturnUrl))
                {
                    return redirectToAdditionalfactorUrl;
                }
                return "~/";
            }
            return null;
